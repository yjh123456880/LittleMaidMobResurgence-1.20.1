  ...
  public void setRotationAngles(
    ...
    if (isWait) {
      ...
      if (aimedBow) {
        ...
        // 副手（非持械手）水平向胸口偏移45°，避免双臂像僵尸一样平直前伸
        float offHandChestOffset = 0.78539816F; // 45°
        // dominantArm 字段对女仆从不被同步（恒为0），必须用真实 cap 值判断主手，
        // 否则会误转主手而非副手。
        if (ModelCapsHelper.getCapsValueInt(pEntityCaps, caps_dominantArm) == 0) {
          // 惯用左手：主手为左臂，副手为右臂，右臂向身体中线（胸口）收拢
          bipedRightArm.addRotateAngleY(-offHandChestOffset);
        } else {
          // 惯用右手：主手为右臂，副手为左臂，左臂向身体中线（胸口）收拢
          bipedLeftArm.addRotateAngleY(offHandChestOffset);
        }
    ...
    // 持盾格挡：左手（副手持盾）抬到身前，右手微收；盾牌由手持物品层跟随手臂渲染
    if (ModelCapsHelper.getCapsValueBoolean(pEntityCaps, caps_isBlocking)) {
      bipedLeftArm.setRotateAngle(-1.4F, 0.3F, -0.15F);
      bipedRightArm.setRotateAngle(bipedRightArm.getRotateAngleX() * 0.5F - 0.35F, 0F, 0F);
    }
  }
