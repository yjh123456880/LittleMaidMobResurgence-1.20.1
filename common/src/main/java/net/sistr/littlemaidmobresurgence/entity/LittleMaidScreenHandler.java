...
public class LittleMaidScreenHandler extends ScreenHandler
    ...
    private final int backpackUpgradeLevel;
    ...
    /** 添加女仆主背包槽位，拒绝放入扩容道具（扩容道具仅可放入专属扩容槽位）。 */
    private void addMaidInventorySlot(int index, int x, int y) {
        addSlot(
                new Slot(maidInventory, index, x, y) {
                    @Override
    ...
    private void layoutMaidInventorySlots() {
        ...
        // index 0~17（扩容道具禁止放入主背包，仅可放入专属扩容槽位）
        int index = 0;
        for (int row = 0; row < 2; row++) {
            for (int col = 0; col < 9; col++) {
                addMaidInventorySlot(index++, 8 + col * 18, 76 + row * 18);
            }
        }
